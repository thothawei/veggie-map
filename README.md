...
- **同義詞與詞綴展開**：搜尋詞會展開成一組同義變體（中／英／日），並剝掉台灣常見的
  詞綴再查一次表——「素食便當」查得到便當、「麵店」查得到麵、「珍珠奶茶店」查得到手搖。
  台中 bbox 實測：麵店 0→14、早餐店 0→7、素食火鍋 0→7。
- **匯入地址正規化**：OSM 的台灣地址寫法很雜（郵遞區號開頭、夾里鄰、「臺／台」混用），
  匯入時收斂成一種寫法（見 `App\Support\TaiwanAddress`），日本地址走同一條路不受影響。
