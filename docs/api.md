...
| GET | `/admin/closures` | 疑似歇業的待審訊號，每筆帶 `google_maps_url` | admin |
| POST | `/admin/closures/{id}` | 審核，body `{ "resolution": "confirmed"｜"dismissed" }`；confirmed 會下架 | admin |
...
- **同義詞展開**（詞表在 `config/veggiemap.php` 的 `search.synonyms`）：每個查詢詞會
  展開成一組互為同義的變體，群組**之內** OR、群組**之間**仍然 AND。
  解決的是「使用者打得出來、資料裡卻是別的寫法」——實測 `keyword=珍珠奶茶` 原本
  0 筆，資料庫裡卻有 22 家標成「手搖飲」的店。展開後 22 筆，且 `keyword=手搖`
  結果相同（詞表是群組不是單向映射，雙向有效）。
  其他實測：`素时`（簡體）→ 對到素食、`毛小孩` → 寵物友善、`輪椅` → 無障礙、
  `vegan` ↔ `全素`。**這不是完整的繁簡轉換**，只收了幾個最常見的寫法。
  同義變體與原詞**同分**：使用者搜「珍珠奶茶」時，一家標成「手搖飲」的店就是他要找
  的店，沒有道理排在後面。變體數上限 `search.max_variants`（預設 8）——每多一個變體，
  WHERE 就多七個 LIKE。實測 1 變體 77ms、8 變體 114ms（1159 筆，清 cache 後）。
- **`matched_menu_items`**：命中的是菜色時，結果會帶上是哪幾道（最多三個）。
  搜「拉麵」跳出一家店名沒有那兩個字的店時，不說明看起來像 bug。店名本身就命中
  時不會有這個欄位——多一行只是雜訊。
- **`meta.did_you_mean`**（2026-09）：「你是不是要找…」。**只在 0 筆且有 keyword 時**
  才跑第二段查詢，而且**不自動改寫**——`data` 仍然是使用者原本那個查詢的結果，
  要不要換成建議的詞由他自己點。自動改寫會讓他不知道自己看的是別的查詢的結果，
  改錯時也沒有回頭路（Google 那樣做是因為它有信心分數，我們沒有）。

  相似度用**字元層級**的編輯距離。**不是** PHP 內建的 `levenshtein()`：那是
  byte-based，UTF-8 的中日文一個字佔 3 bytes，2026-09-06 實測「素時 vs 素食」
  給距離 3、「拉麵 vs 拉面」給 2——同樣都只差一個字卻拿到不同數字，門檻因此失去意義。

  **也不是規劃原本寫的「字元 bigram 的 Jaccard」**：兩個字的詞只有一個 bigram，
  一字之差交集就是空集合。同一組候選的對照實測：
  ...
  中文使用者最常打錯的正是兩三個字的詞，bigram 在那個長度上一律回 0。
  字元層級對 ASCII 的行為與內建函式相同，所以不需要中英文兩條路徑。

  候選是四份小清單：**同義詞表的 120 個詞**、料理種類 label、行政區、店名
  （2026-09-06 共約 1393 筆）。同義詞表排最前面是實測後才加的——原本只有後三份時，
  日文使用者打錯的「ラーメソ」拿不到建議，因為候選是完整店名「ラーメン 麺尊 RAGE」，
  跟一個四字查詢的整體相似度太低。實測成本 **4.6–11ms**。
- **`meta.relaxations`**（2026-09）：**只在結果為 0 筆時出現**。零結果是搜尋體驗最貴的
  一刻，使用者當下的問題不是「沒有店」而是「我做錯了什麼」——四個篩選任一個都可能是
  兇手，叫他自己一個一個試等於叫他放棄。所以直接把答案算出來：
  `[{ param, value, label, count }]`，例如
  `{ param: 'confidence_min', value: null, label: '不限可信度', count: 576 }`。
  `value` 為 null＝把該參數移除；有值＝改成那個值（`venue_scope` 是後者，
  移除它前端會退回自己的預設「純素食店」，等於按了沒反應）。
  只列 `count > 0` 的，最多三項——放寬後仍然 0 筆的條件不是有用的建議。
  **成本**：每一項是一次乾淨的 `COUNT(*)`（沒有排序、沒有 eager load），
  2026-09-06 實測四個條件全開是 **32–41ms**，低於規劃設的 150ms 門檻，因此四項全算。
- **`meta.expanded_terms`**（2026-09）：展開原本是隱形的——使用者搜「珍珠奶茶」
  看到一家叫「綠意茶飲」的店排第一，只會覺得搜尋不準。回應現在會說「也一併搜尋了」
  哪些變體：`[{ term: '珍珠奶茶', variants: ['珍奶','奶茶','手搖','手搖飲','飲料',…] }]`
  ——**只列真的被拿去查的那些**（已套用 `max_variants` 截斷），原詞不重複列進
  `variants`。沒有任何詞展開時整個 key 不出現。
- **`exact=1`**（2026-09）：只用原詞查，不展開。展開幫倒忙時的逃生門（搜「麵包」
  不想看到一整頁麵店）。**仍然會斷詞**：「台中 拉麵」依舊是兩個 AND 條件，只是不帶
  變體——exact 是「不要展開」，不是「換一種斷詞方式」。帶 `exact=1` 時不回
  `expanded_terms`（畫面說的必須跟查詢做的一致）。
  2026-09-06 實測（612 筆資料，`venue_scope=all`）：`拉麵` 展開 25 家 → exact 24 家，
  少掉的那一家是純粹靠日文變體「ラーメン」才命中的；`珍珠奶茶` 展開 23 家 → exact **0 家**
  （沒有任何一家店的名字真的叫「珍珠奶茶」，這 23 家全部是展開的功勞）。
