...
- [ ] **單字剝離擴散**：搜「素時」（打錯的「素食」）會被詞綴剝離剝成「時」，
      回傳 14 家名字含「時」的店（六扇門時尚湯鍋、IAM澳洲小時光…），畫面還說
      「也一併搜尋了：時」。**這比 0 筆更糟**——使用者以為找到了，但全都不是他要的。
      A4 幫不上忙（它只在 0 筆時觸發）。可能的方向：剝完只剩**單一 CJK 字元**時
      不當作變體（「素食滷味」→「滷味」要留，「素時」→「時」要丟），
