...
測試：後端 **634**（4 skipped，1736 assertions）、前端 **316**，PHPStan 0 error、Pint PASS、CI 綠
...
**還可以做的**：`ベジタリアン` 只有 1 筆，因為 `diet_types` 沒有被關鍵字比對——
使用者搜「素食」時對不到 diet 標籤，只能靠店名與 feature label。要不要把 diet
納入關鍵字比對範圍是產品決定（納入後「素食」會回傳幾乎所有店，可能反而沒用）。
