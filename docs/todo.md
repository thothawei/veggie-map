...
- [ ] **首頁沒有手動排序控制項**：列表頁有「相關性／素食可信度／最新收錄」
      的排序選單，首頁沒有——排序完全由前端自動規則決定（關鍵字→相關性、
      有座標→距離、否則→最新收錄），使用者改不了。B1 的底部 sheet 現在
      會顯示搜尋結果清單了，補一個排序選單的價值比 B1 之前（只有地圖跟
      markers）更高。不算 bug（跟規劃前的行為一致），是新增的機會。
