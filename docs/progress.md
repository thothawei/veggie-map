...
後端 570 綠、前端 298 綠、PHPStan 0、Pint PASS。

**開發庫已回寫**（使用者看過 dry-run 報表後同意）：967 筆換成拼音、967 筆別名。
瀏覽器實測：`/restaurants/osm-2296283853`（舊）載出「十方齋素食館」而且網址自動變成
`/restaurants/shi-fang-zhai-su-shi-guan`；`/restaurants/23` 維持數字不換；亂打的 slug
仍然 404。（第一次測沒換，原因是 `public/build` 還是 11:12 的舊資產，`npm run build`
後才是真的——前端改動的驗收要記得先重建。）
...
## 2026-09-06 A2 — 同義詞展開要看得見，而且可以關掉 ✅ 已完成

**先誠實補一件事**：A1 的驗收條件寫的是「搜一個只會命中料理種類的詞，**卡片顯示**
『料理種類：日式拉麵』」，但上一輪只做了後端欄位，前端卡片還在讀舊的
`matched_menu_items`。這一輪一併補上（`formatMatchReasons()` ＋ 卡片改讀
`matched_reasons`），A1 到這裡才真的算完成。

### 後端

- `KeywordSearch::groupsFor(?string $keyword, bool $exact)` 是「這個查詢實際上會用
  哪些詞去搜」的**單一入口**。WHERE、相關性排序、命中原因、meta 全部從這裡拿——
  Controller 與 Repository 各算一次的話，`exact` 遲早只在其中一邊生效，那時候
  「畫面說也搜了手搖飲、查詢其實沒搜」就會發生。它是純函式（只讀 config、不碰 DB），
  所以 Controller 重複呼叫一次的成本可以忽略，**不需要為它去改動 `search()`
  已經被 cache 起來的回傳形狀**。
- `KeywordSearch::expandedTerms()` 產生 `meta.expanded_terms`：只列**真的被拿去查**
  的變體（已套用 `max_variants` 截斷），原詞是 `term`、不重複列進 `variants`。
  沒有展開就整個 key 不出現——回空陣列會逼前端多寫一層「有 key 但它是空的」判斷，
  而那跟「沒展開」是同一件事。
- `exact` 走 `SearchRestaurantRequest`，並跟 `open_now` 一樣收 axios 的
  `"true"`／`"false"`（原本那段寫死 open_now 一個，改成兩個共用一個迴圈）。
- **exact 仍然斷詞**：「台中 拉麵」在 exact 模式下依然是兩個 AND 條件，只是不帶變體。
  整串當一個詞會讓 exact 從「不要展開」變成「換一種斷詞方式」，那是兩件不同的事
  （有測試守著）。

### 前端

- 結果頁頂端「『珍珠奶茶』也一併搜尋了：珍奶 奶茶 手搖 …」，每個變體可點（改搜那個詞）；
  旁邊「只搜「珍珠奶茶」」寫 `?exact=1` 進網址。exact 模式下換成另一行
  ＋「也搜同義詞」的回頭路。
- `exact` 進網址而不是只存元件狀態：否則重新整理或分享連結時，畫面說「只搜原詞」
  但結果其實是展開過的——那比不做這個開關更糟。
- 清除關鍵字時一併清掉 `exact`（沒有關鍵字它沒有意義，留著會在下一次搜尋悄悄生效）。
- `formatMatchReasons()`：同型別合併成一行（「命中菜色：味噌拉麵、醬油拉麵」），
  `name` 型別刻意不顯示——店名就印在同一張卡片第一行，再說一次只是雜訊
  （沿用 `matched_menu_items` 時代就有的判斷）。

### 量到的數字（2026-09-06 真的打本機 API，612 筆資料）

| 查詢 | 展開 | `exact=1` | 說明 |
|---|---|---|---|
| 珍珠奶茶（venue_scope 預設） | 23 家 | **0 家** | 沒有一家店真的叫「珍珠奶茶」，這 23 家全是展開的功勞 |
| 拉麵（venue_scope=all） | 25 家 | 24 家 | 少掉的那家是純粹靠日文變體「ラーメン」才命中的 |
| 麵包 | 1 家 | 1 家 | 展開到 bakery／パン／ベーカリー，**沒有**擴散到麵店——2026-08-31 那條防線仍然守著 |

### 驗證

- 反向驗證兩次：後端把 `if ($exact)` 換成 `if (false)`，「exact 讓結果嚴格變少」與
  「exact 不掛展開說明」兩條紅；前端拿掉送出 `exact` 參數那一行，兩條前端測試紅。
- 真瀏覽器實測（`localhost:8080`，已 `npm run build`）：搜「珍珠奶茶」看到那一行與
  可點的變體；卡片顯示「料理種類：飲料／手搖」——**A1 的驗收條件到這裡才真的成立**
  （麻古茶坊、50嵐 店名都沒有「珍珠奶茶」，現在說得出是命中料理種類、經由變體「手搖」）；
  點「只搜「拉麵」」→ 網址加上 `exact=1`、那一行換成「只搜「拉麵」，沒有一併搜尋同義詞。」
- 後端 658 passed（4 skipped、1800 assertions）、前端 326 passed、PHPStan 0、Pint PASS、
...
## 2026-09-06 A3 ＋ B6 — 零結果給「放寬哪一個條件會有幾家」 ✅ 已完成

A3（後端算答案）與 B6（畫面變成可按的下一步）是同一件事的兩半，一起做。

### 規格上調整了一處，理由是實作時才看得到的

規劃寫的回傳形狀是 `{ drop, label, count }`，但 **`venue_scope` 的放寬不是「移除參數」
而是「改成 all」**：前端把它從網址移掉的話，`useFilterQuery` 會退回自己的預設值
（純素食店），使用者按了等於沒反應。所以形狀改成 `{ param, value, label, count }`，
`value === null` 才是移除。有測試釘住這件事。

同理，**`bbox` 在列表頁要翻譯成 `city`**：那一頁的 bbox 是從 `?city=` 算出來的，
網址上根本沒有 bbox 參數，照字面刪它會變成一個按了沒反應的按鈕。

### 成本：先量再定

規劃說「若量測顯示超過 150ms，就砍成只算前兩個。**先量再定**」。
實測（四個條件全開、台中 bbox、暖機後三次）：**32.5 / 41.5 / 39.9 ms**。
遠低於門檻，所以四項全算，不砍。

每一項是一次乾淨的 `COUNT(*)`（`countFor()`：沒有排序、沒有 eager load、沒有
cursor），而且 **Controller 只在 `$paginator->items() === []` 時才呼叫**——
有結果的查詢跑它是純粹的浪費。

### 演算法是誠實的（實測佐證）

只列 `count > 0` 的。真實資料上：`open_now=1` 單獨有 43 筆（有結果，所以不算
relaxations）；`open_now=1&confidence_min=30` 是 0 筆，而它**沒有**建議「不限營業中」
——因為拿掉 open_now 後仍然是 0 筆（沒有店有 30 分）。它只建議真正有效的那一個
「不限可信度（43 家）」。不會謊稱「拿掉這個就有店了」。

### 驗證

- 後端 6 條、前端 6 條測試。反向驗證兩邊都做：`relaxations()` 直接 `return []`
  → 後端 3 條紅；前端 `v-if="false"` → 4 條紅。
- 規劃的驗收情境（開「營業中」→ 0 筆 → 按鈕說 N 家 → 按下去真的有 N 家）由
  `test_open_now_relaxation_count_matches_what_you_actually_get` 精確覆蓋
  （用「店家沒有營業時間資料」造出 open_now 是唯一兇手的情境，不依賴半夜）。
  **真瀏覽器實測走的是 confidence_min 那條路**（10:53 有 43 家營業中，湊不出
  「open_now 是唯一兇手」）：`?confidence_min=60` → 0 筆 → 畫面出現
  「不限可信度（576 家）」→ 按下去網址移除該參數、結果真的出現。
  同一個機制的兩條路徑，這點誠實記錄。

**下一步**：A4（真的錯字容錯：「你是不是要找…」，CJK 用 bigram 而不是 levenshtein）。

## 2026-09-06 A4 — 真的錯字容錯：「你是不是要找…」 ✅ 已完成

### 規劃的兩個前提，實測後都不成立

**其一：「素時」不是 0 筆，是 14 筆。** 規劃寫「真的打錯（`vegitarian`、`素時`、
片假名打錯一個字）仍然是 0 筆」，但 2026-08-31 加的詞綴剝離把「素時」剝掉「素」
剩下「時」，於是搜出 14 家名字含「時」的店：六扇門時尚湯鍋、IAM澳洲小時光、
時蔬異理義大利麵……而且畫面還說「也一併搜尋了：時」。**這比 0 筆更糟**——
使用者以為找到了，但那些完全不是他要的。這一項**不在 A4 範圍內**（A4 是 0 筆才觸發），
已另外記在待辦，見下方「單字剝離擴散」。

**其二：規劃指定的 bigram 演算法對短詞無效。** 規劃說「CJK 用字元 bigram 的
Jaccard 相似度」，理由（`levenshtein()` 是 byte-based，對 UTF-8 中日文給出無意義的
距離）**完全正確**，但開的藥方不對：兩個字的詞只有一個 bigram，一字之差交集就是
空集合。同一組候選的對照實測：
...
照規劃字面實作的話，連規劃自己寫的驗收條件（「素時」要建議「素食」）都達不到。
**改用字元層級的編輯距離**：先切成字元陣列再算，順序資訊因此保留；而且它對 ASCII
的行為跟內建 `levenshtein()` 完全一致（ASCII 一字元就是一 byte），所以**不需要
中英文兩條路徑**，比規劃的設計更簡單。

### 候選集合：實測後多加了一份

原本照規劃只有三份（店名／料理種類 label／行政區），結果日文使用者打錯的
「ラーメソ」拿不到任何建議——候選是**完整店名**「ラーメン 麺尊 RAGE」，跟一個
四字查詢的整體相似度太低，還會先被長度比例那一刀砍掉。

加上**同義詞表的 120 個詞**（它本來就是「使用者會打的詞」的清單）之後：

| 查詢 | 加之前 | 加之後 |
|---|---|---|
| ラーメソ | （無建議） | **ラーメン(0.75)** |
| vegitarian | Vegetarian Beast(0.56) ← 店名 | **vegetarian(0.9)** ← 正確的詞 |
| 素時 | 素食(0.5) | 素食(0.5)、素时(0.5) |
| xyzzy | （無建議） | （無建議）✓ 不亂猜 |

成本 **4.6–11ms**，只在 0 筆且有 keyword 時才跑。

### 不自動改寫

`data` 仍然是使用者原本那個查詢的結果（0 筆），`did_you_mean` 只是建議。
自動改寫會讓他不知道自己看的是別的查詢的結果，改錯時也沒有回頭路。有測試釘住
「有建議不代表要替使用者改查詢」。前端也一樣：點下去才換網址。

畫面上「你是不是要找」排在放寬條件按鈕**之前**——打錯字時錯字才是真正的原因，
先叫使用者去放寬篩選等於指錯方向。有錯字建議時就不再顯示靜態的「換個關鍵字」，
那是同一件事的兩種講法。

### 驗證

- 後端 9 條、前端 3 條新測試。反向驗證：把 `distance()` 換回內建 `levenshtein()`
  → **中日文 4 條紅、ASCII 那條仍綠**，正好證明 byte-based 只壞在 CJK；
  前端不渲染 → 2 條紅。
- 真瀏覽器實測：`?keyword=拉面` → 0 筆 → 「沒有符合「拉面」的餐廳。你是不是要找
  拉麵？」→ 網址上仍是「拉面」（沒有自動改寫）→ 點下去才換成「拉麵」並出現結果。
...
## 2026-09-06 — A5：搜尋範圍變成明確的控制項

**問題**：首頁「不限目前範圍」、列表頁「跨全部城市」都是藏在程式邏輯裡的特例
（一打關鍵字就強制忽略目前的視角／城市），使用者看不到也改不了，兩頁行為還不一樣。

**做什麼**

- `useSearchScope.ts`：`?scope=map|city|all` 的單一真相來源，跟 `useCities`／
  `useFilterQuery` 同一套約定——是預設值時網址上不寫這個參數。首頁預設 `map`，
  列表頁預設 `city`（跟兩頁原本「沒有關鍵字」時的行為一致，改變的只有「打了
  關鍵字」那個分支）。
- **拿掉自動特例，不是加一層蓋在上面**：首頁原本關鍵字搜尋一律不送 bbox；
  列表頁原本（2026-08-25 的決定）一打關鍵字就強制跨全部城市。這兩個都拿掉了，
  換成使用者自己選的 `scope`——這是真的行為改變：列表頁預設現在會把關鍵字
  搜尋限定在目前城市，除非使用者自己切到「全部城市」。
- `ScopeSelect.vue`：兩頁共用的選單元件，`options` 由呼叫端決定露出哪幾個
  （首頁三個都有；列表頁沒有地圖，只給 `city`／`all`，網址上出現 `scope=map`
  時退回跟 `city` 一樣的處理）。
- **`scope=all` 完全不能送座標**：後端在沒有 bbox 時，lat/lng 會套上預設 5km
  半徑（`RestaurantRepository::search()`），把「不限城市」悄悄變回「原地」——
  這是 A5 首頁那半原本就有的既定坑，這次沿用同一個防法。有 bbox（`city`／`map`）
  時座標不受這個限制，一起送只是多算一個 distance 欄位。
- 首頁「推薦餐廳」故意跟搜尋範圍脫鉤，一律看目前地圖視角——選了「全部城市」
  不代表使用者想看畫面外城市的推薦。

**驗證**

- 前端新增 `useSearchScope.test.ts`（6 條）；`HomeView.test.ts`／
  `RestaurantListView.test.ts` 補了 scope 相關案例、修正舊的「關鍵字自動跨城市」
  斷言；全套 365 條全綠，eslint／vue-tsc 乾淨。
- 反向驗證：拿掉 `useSearchScope` 的網址回填 → 3 條紅；拿掉 `bbox` 的 scope 判斷
  （列表頁）→ 3 條紅。
- 真瀏覽器（localhost:8080）：首頁選「全部城市」→ 網址變 `?scope=all` →
  拖動地圖 → 計數維持「100+ 家（全部城市）」沒有被視角收窄 → network 確認
  `/restaurants` 請求不帶 `bbox`（`/restaurants/recommended` 仍帶，符合設計）→
  重新整理後選單與計數都還原。列表頁台中＋「素食」預設限定台中，切到
  「全部城市」後 `.global-hint` 顯示「範圍是全部城市，不受目前選的「台中」限制」，
  結果從限定台中變成 20+ 家。
...
## 2026-09-06 — B1：首頁改地圖優先版面

**做什麼**：移除「VeggieMap／找到適合你的素食餐廳」hero 標題（品牌留在
header 就夠），控制項（城市切換／搜尋框／範圍選單／篩選）改成浮動列蓋在
地圖上，不再把地圖推到下面；新增底部 sheet 顯示搜尋結果清單（`restaurants`，
不是「推薦餐廳」）——這是首頁第一次能看到結果清單，不是只有地圖上的點。

- 行動版：sheet 收合時只有一行摘要＋展開按鈕，展開後看得到卡片清單。
  **簡化**：規劃寫的是「收合／半開／全開」三段拖曳手勢，這裡做成收合／
  展開兩態的按鈕——拖曳手勢的實作與測試成本遠高於一個按鈕，兩態已經解決
  「手機上控制項吃掉大半螢幕」的核心問題，拖曳可以是之後的漸進增強。
- 桌機（≥900px）：sheet 變成常駐的左側清單（360px）＋地圖兩欄並排，
  `sheet-toggle` 隱藏、`sheet-body` 用 `display:block!important` 蓋掉
  `hidden` 屬性，不用 JS 判斷視窗寬度。
- 兩種情況自動展開 sheet，不用使用者自己點：①打關鍵字且有結果——使用者
  主動要找東西，看得到命中清單比看地圖上一堆點更直接；②零結果——空狀態
  的說明（放寬條件／你是不是要找）該一目了然，逼人先展開才看得到理由是
  多一次無意義的點擊。使用者手動收合／展開之後不會被這兩條規則打回去。
- `.sheet-body` 用 `hidden` 屬性而不是 `v-if` 控制顯示——內容留在 DOM 裡，
  桌機版常駐顯示才有東西可以直接用 CSS `!important` 蓋掉。

**踩到的坑**：桌機版 `.map-legend` 的 `left:380px`（清單佔走左側後把圖例
挪到清單外面）完全沒生效，因為 `@media (min-width:900px)` 那個區塊寫在
`.map-legend` 的基礎規則**之前**——CSS 同 specificity 時後面的規則贏，
基礎規則因為在檔案裡排在媒體查詢後面，把它蓋回去了。**修法：把所有
媒體查詢區塊移到 `<style>` 最後**，不管它要覆蓋的基礎規則寫在檔案哪裡。
真瀏覽器用 `getBoundingClientRect()` 量出來 x:12（沒生效）才抓到，光看
CSS 原始碼看不出來——這條規則本身完全沒有語法錯誤。

**範圍縮減（記在 todo.md 的新發現）**：沒有加手動排序選單到首頁，沿用
既有的自動排序規則。加一個要跟網址同步的排序控制是比排版更大的一塊範圍，
留給以後。

**驗證**

- 前端新增 8 條測試（`HomeView 地圖優先版面（B1）`），全套 382 條全綠；
  eslint／vue-tsc／`npm run build` 乾淨。
- 反向驗證：拿掉兩個自動展開的 watch → 2 條紅。
- 真瀏覽器：390×844 手機尺寸，搜「素食」→ sheet 自動展開顯示命中清單
  （素食、素食饗、素食Homie蔬適廚房…帶距離／地址）；點收合列可以手動收合
  （`#result-sheet-body` 的 `hidden` 屬性正確切換）。1200×800 桌機尺寸，
...
## 2026-09-06 — 修掉單字剝離擴散（做 A4 時發現的坑）

**問題**：`KeywordSearch::lookupForms()` 剝詞綴時沒有限制剝完的長度。「素時」
（打錯的「素食」）不在同義詞表裡，會被剝掉前綴「素」剩下「時」——那不是任何
食物詞，純粹是巧合的單一字元，卻被當成合法變體加進查詢，讓搜尋擴散成
「店名有『時』的店都算」，回傳一堆不相干的店（六扇門時尚湯鍋、IAM澳洲小時光
…）。**這比 0 筆更糟**：0 筆使用者知道要換詞，這種情況他以為找到了。

**修法**：剝完只剩**單一 CJK 字元**時，那個字必須本身就是同義詞表收錄的詞
（例如「麵」，見 `['麵','noodle','麵食','麺']` 那組）才留著當變體，不然視同
剝離失敗，改試下一個詞綴。兩字以上的剝離不受影響——「滷味」不在任何詞表
也照樣留著，維持既有行為。抽出 `isKnownWord()` 共用判斷式，跟函式開頭
「詞表裡本來就有的詞不剝」那條規則是同一個判準，不是另外發明一套。

**驗證**

- `tests/Unit/KeywordSearchTest.php` 新增 3 條：「素時」不再展開出「時」、
  「麵店→麵」不受影響（麵本身在詞表裡）、「素食滷味→滷味」不受影響
  （兩字以上不受這條限制）。後端全套 710 條全綠，Pint／PHPStan 乾淨。
- 反向驗證：拿掉單字元檢查那段 → 新增的「素時」測試立刻紅。
- 真環境：`curl '.../restaurants?keyword=素時'` → `data:[]`、
  `meta.did_you_mean` 正確給「素食」「素时」，沒有 `expanded_terms`
  （因為「時」不再是變體）；`tinker` 直接呼叫 `KeywordSearch::expand(['素時'])`
  回傳 `[['素時']]`，不再夾帶「時」。

**這是這一批（plan-2026-09-search-ux.md）之外、做 A4 時額外發現並修掉的
真實 bug，不是規劃項目**。todo.md 記的另一項發現（首頁沒有手動排序控制項）
是範圍決定不是 bug，維持待評估狀態。

## 2026-09-06 — 補上首頁排序控制項（B1 發現的缺口）

**做什麼**：首頁在 B1 之前排序完全由前端自動規則決定（有關鍵字→相關性、
有座標→距離、否則→最新收錄），使用者改不了；B1 加了底部 sheet 顯示結果
清單後，這個缺口的價值變高了（有清單可以排序，不只是地圖上的點）。這次
比照列表頁補上手動選單：

- 選項比列表頁多一個「距離」——首頁有地圖視角可以算距離，列表頁用 bbox
  收窄沒有中心點，這是兩頁本來就不同的地方，不是硬要對齊。
- 可用選項動態增減：「相關性」只在有關鍵字時出現，「距離」只在有座標
  （`scope !== 'all'`）時出現——跟列表頁的 `availableSorts` 同一套邏輯，
  避免使用者選到一個會讓後端回 422 的組合。
- 網址是真相來源，等於預設值就不寫進網址，跟 `useSearchScope`／
  `useFilterQuery` 同一套約定。
- 選單放在底部 sheet 的收合列裡（跟規劃 B1 的 ASCII 版面圖「42 家 ·
  相關性 ▾」原本畫的位置一致），跟 `sheet-toggle` 分開成兩個元素——
  `<select>` 是互動元素，巢狀在 `<button>` 裡是無效 HTML，點下拉會被
  外層按鈕的 click 一起吃掉。

**驗證**

- 新增 5 條測試：預設值隨座標／關鍵字變化、選單選項動態增減、手動選排序
  寫進網址且重新查詢、選回預設值時網址參數被清掉、網址帶不可用排序時
  退回預設。全套 419 條全綠，eslint／vue-tsc／`npm run build` 乾淨。
- 反向驗證：把送出去的 `sort` 參數硬寫成固定值 → 3 條紅。
- 真瀏覽器：選單正確顯示「距離／素食可信度／最新收錄」（無關鍵字時）；
  手動切成「素食可信度」→ 網址變 `?sort=confidence`，`network` 記錄確認
  下一次 `/restaurants` 請求的 `sort` 參數從 `distance` 變成 `confidence`。
...
標註「可能是合理裁決，但裁決沒有被寫下來」，不是像前三項一樣單純的規格
缺口，而是待辦自己要求「決定不做就寫明理由」的一項。

## 2026-09-06 — 裁決：`Projects`／`Tasks`／`Settings` 三頁不做

**做的事**：todo.md 對這一項的措辭是「這一項可能是合理裁決，但裁決沒有
被寫下來……決定不做的話，就在這裡改成 `[~]` 並寫明理由」——這跟前面三項
（ResourceUsage／AgentDetailView／LogsView）不同，那三項是單純的規格缺口，
這項是待辦自己要求先做判斷。分開查證三個子項，不是用一句「MVP 用不到」
籠統帶過：

1. **`Projects` 清單頁**：`CommandCenter`（掛在 Dashboard）已經是列出全部
   專案＋建立新專案的完整功能，只是沒有獨立路由。目前專案數是個位數
   （seeder／Demo 產生的），分頁／篩選都用不上，另開一條路由只是同一份
   資料換一個網址，沒有新增能力，純粹是為了規格頁面清單字面對齊。
2. **`Tasks` 跨專案頁**：這一輪剛做完的 `LogsView` 已經證明「跨專案瀏覽＋
   篩選」在架構上完全不是障礙（照抄同一套模式就能做）。沒做的原因是
   **沒有需求信號**——現在看任務都是先進某個專案（`TaskBoard`），還沒
   出現「跨專案找某個狀態的任務」這種使用情境。這是跟 B3／B4 quick filter
   同一種判斷：沒有真實使用資料支持，先做就是用猜的湊功能，跟這個 repo
   一貫的「先量再做」原則衝突。
3. **`Settings` 頁**：查證後**沒有任何東西可以放進去**——AI Office 的設定
   全部是 `.env`／`config/ai_office.php`（`DASHBOARD_ALLOWED_EMAILS`、
   sandbox 上限、LLM provider 等），改了要重新部署，不是執行期可調的
   使用者設定。做一個空頁只是讓路由清單看起來完整，不是使用者需要。

**寫下會讓這個裁決過期的條件**（不是丟著不管）：專案數成長到需要分頁／
搜尋、有人明確要求跨專案任務搜尋、或新增任何一個執行期可調的設定項——
三個條件任一觸發，這一項就要重新打開來做，不是永久關閉。

**驗證**：這是文件決策，沒有程式碼改動，不需要跑測試。查證動作是讀
`CommandCenter.vue`／`ProjectDetailView.vue`／`config/ai_office.php`／
`.env.example` 確認上述三點屬實，不是憑印象寫的。
